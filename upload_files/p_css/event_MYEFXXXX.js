...
if (!document.getElementsByClassName('c_tooltips01').length) {
    ...
    function setBalloon() {
        ...
        for (let i = 0; i < tooltip.length; i++) {
            ...
            // テキストエリアのleftを取得
            const leftRect = textAreaRect.left + margin;
    ...
    function c_tooltipSetPosition() {
        ...
        for (let i = 0; i < tooltip.length; i++) {
            ...
            switch (IconWidth) {
                case 24:
                    triangle.style.left = tooltipRect.left - 4 + 'px';
                    textArea.style.left = tooltipRect.left + IconWidth / 2 - textAreaWidth / 2 + 'px';
                    break;
                case 32:
                    triangle.style.left = tooltipRect.left + 'px';
                    textArea.style.left = tooltipRect.left + IconWidth / 2 - textAreaWidth / 2 + 'px';
                    break;
                default:
                    triangle.style.left = tooltipRect.left + IconWidth / 2 - triangle.offsetWidth / 2 + 'px';
                    textArea.style.left = trianglePosition.left - textAreaWidth / 2 + triangle.offsetWidth / 2 + 'px';
            }
            // ツールチップの大きさ指定
...
window.addEventListener('DOMContentLoaded', function () {
    ...
    for (let i = 0; i < radioList.length; i++) {
        ...
        if (radioList[i].parentElement.classList.contains('c_radio03_disabled')) {
            radioList[i].children[1].setAttribute('tabindex', '-1');
        }
...
window.addEventListener('DOMContentLoaded', function () {
    ...
    for (let i = 0; i < checkBoxList.length; i++) {
        ...
        // 該当の要素がdisableならフォーカス不可とする
        if (checkBoxList[i].parentElement.classList.contains('c_checkBox01_disabled')) {
            checkBoxList[i].children[1].setAttribute('tabindex', '-1');
        }
...
if (!document.getElementsByClassName('c_toggle01').length) {
    ...
    window.addEventListener('DOMContentLoaded', function () {
        ...
        for (let i = 0; i < toggleList.length; i++) {
            ...
            // キー操作が行われたら処理を実行
            toggleList[i].children[2].addEventListener('keydown', function (e) {
